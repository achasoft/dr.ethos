...
export class GridComponent<T> implements OnInit, OnDestroy, AfterContentInit {
  ...
  ngAfterContentInit(): void {
    if (this.cultureService.lang !== 'en') {
  ...
  async updateDataSource(pageChanged: boolean = false): Promise<void> {
    if (!this.backend) {
      return;
    }
    this.isLoading = true;
    ...
    const query = {} as any;
    query[this.filterField || 'name'] = (this.query || '').trim();
    const op = (await this.httpService.grid<T>({
      ...
      pageNumber: (this.currentPage || 1) - 1,
  ...
  async onCommand(command: GridCommand<any>): Promise<void> {
    if (command.goToPage) {
